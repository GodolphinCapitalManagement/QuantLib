...
#include <ql/experimental/loans/amortizingfixedrateloan.hpp>
...
namespace QuantLib {
    ...
    namespace  {
        ...
        std::vector<Real> sinkingNotionals(const Period& maturityTenor,
            ...
            QL_REQUIRE(isSubPeriod(freqPeriod, maturityTenor, nPeriods),
                       "Loan frequency is incompatible with the maturity tenor");
    ...
    AmortizingFixedRateLoan::AmortizingFixedRateLoan(
                                      ...
                                      BusinessDayConvention paymentConvention,
                                      const Date& issueDate)
    : Loan(settlementDays, calendar, issueDate),
      frequency_(sinkingFrequency),
      dayCounter_(accrualDayCounter) {

        QL_REQUIRE(loanTenor.length() > 0,
                   "loan tenor must be positive. "
                   << loanTenor << " is not allowed.");
        maturityDate_ = startDate + loanTenor;

        cashflows_ =
            FixedRateLeg(sinkingSchedule(startDate, loanTenor,
                                         sinkingFrequency, calendar))
            .withNotionals(sinkingNotionals(loanTenor,
